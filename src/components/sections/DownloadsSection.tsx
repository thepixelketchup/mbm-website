...
export default function DownloadsSection({ section, allDocuments = [] }: DownloadsSectionProps) {
    ...
    const getCategoryIcon = (category: string) => {
        ...
        return icons[category as keyof typeof icons] || <FaFileAlt className="w-8 h-8" />
    }
    ...
    const getCategoryColor = (category: string) => {
        ...
        return colors[category as keyof typeof colors] || 'from-indigo-500 to-indigo-600'
    }
    ...
    const groupedDocuments = documentsToShow.reduce((acc, doc) => {
        if (!acc[doc.category]) {
            acc[doc.category] = []
        }
                ...
                <div className="max-w-7xl mx-auto px-6">
                    ...
                    {Object.keys(groupedDocuments).length === 0 ? (
                        <div className="text-center py-16">
                            ...
                            <h3 className="text-2xl font-semibold text-gray-600 mb-2">No Documents Available</h3>
                            <p className="text-gray-500">Please check back later for updated documents.</p>
                        </div>
                    ) : (
                        Object.entries(groupedDocuments).map(([category, docs]) => (
                            <div key={category} className="mb-12">
                                ...
                                {/* Category Header */}
                                <div className={`bg-gradient-to-r ${getCategoryColor(category)} rounded-2xl p-6 mb-6`}>
                                    ...
                                    {docs.map((doc) => (
                                        <div key={doc._id} className="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 p-6 group hover:-translate-y-2">
                                            ...
                                            <div className="flex items-start justify-between mb-4">
                                                <div className={`w-12 h-12 bg-gradient-to-r ${getCategoryColor(category)} rounded-lg flex items-center justify-center`}>
                ...
                <section className="py-8 bg-indigo-600 text-white">
                    <div className="max-w-4xl mx-auto px-6 text-center">
                        <p className="text-indigo-100">
                            📄 This page automatically updates when new documents are added to our admissions section
                        </p>
                    </div>
                </section>
            )}
