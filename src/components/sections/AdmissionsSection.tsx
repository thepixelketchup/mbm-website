...
export default function AdmissionsSection({ section }: AdmissionsSectionProps) {
    ...
    const groupedDocuments = section.documents.reduce((acc, doc) => {
        if (!acc[doc.category]) {
            acc[doc.category] = []
        }
                    ...
                    {Object.entries(groupedDocuments).map(([category, docs]) => (
                        <div key={category} className="mb-12">
                            ...
                            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {docs.map((doc) => (
                                    <div key={doc._key} className="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 p-6 group hover:-translate-y-2">
