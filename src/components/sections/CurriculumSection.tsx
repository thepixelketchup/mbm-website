...
export default function CurriculumSection({
  ...
  // 1) if autoSyncDocuments true -> show allDocuments
  // 2) else if selectedDocuments provided -> show those
  // 3) else -> empty list
  const documentsToShow: CurriculumDocument[] = React.useMemo(() => {
    if (section.autoSyncDocuments) return allDocuments ?? []
    if (Array.isArray(section.selectedDocuments) && section.selectedDocuments.length > 0) {
  ...
  return (
    ...
    >
        ...
        <div className="max-w-6xl mx-auto px-6">
          ...
          {documentsToShow.length > 0 ? (
                ...
                >
                  <div className="flex-1">
                    ...
                    <div className="mt-3 text-xs text-gray-500">
                      {doc.fileType && <span className="mr-2 uppercase tracking-wide">{doc.fileType}</span>}
                      {doc.uploadedAt && (
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    ...
                    >
                      ...
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v12m0 0l4-4m-4 4l-4-4M21 21H3" />
                      </svg>
                    </a>
            ...
            <div className="rounded-lg p-6 border border-dashed border-gray-200 text-center">
              <p className="text-gray-600">No curriculum / admission documents are available right now.</p>
            </div>
          )}
        </div>
      </section>
      ...
      {Array.isArray(section.gradeWiseCurriculum) && section.gradeWiseCurriculum.length > 0 && (
        <section className="py-16 bg-white">
          <div className="max-w-6xl mx-auto px-6">
            <h2 className="text-3xl md:text-4xl font-bold text-indigo-700 mb-8">Grade-wise Curriculum</h2>

            <div className="space-y-8">
              {section.gradeWiseCurriculum.map((grade) => (
                <div key={grade.grade} className="bg-gray-50 rounded-lg p-6 border">
                  <h3 className="text-xl font-semibold text-gray-800 mb-3">{grade.grade}</h3>

                  {Array.isArray(grade.items) && grade.items.length > 0 ? (
                    <ul className="space-y-3">
                      {grade.items.map((item, idx) => (
                        <li key={item.id ?? `${grade.grade}-${idx}`} className="flex items-start justify-between">
                          <div>
                    ...
                    <p className="text-sm text-gray-500">No items listed for this grade.</p>
                  )}
                </div>
              ))}
            </div>
